...
public class StormUtilsTest {
    ...
    private void submitWithSpout(String name, List<String> args, Number parallelism) {
        try {
            StormUtils.submit(builder, name, args, config, parallelism);
        } catch (Exception ignored) {
        }
    }
    ...
    public void testFailingSubmitOnMissingSpout() throws Exception {
        ...
        utils.submit("non.existant.spout", null, new BulletStormConfig(), null);
    }
    ...
    public void testHookingInSpout() {
        Assert.assertFalse(builder.isTopologyCreated());
        submitWithSpout(CustomIRichSpout.class.getName(), null, 2);
